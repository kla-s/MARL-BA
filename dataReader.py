...
def getInitCoords(filename):
    """expects the file to have lines in the same order as the landmark files, one absolute path per line"""
    with open(filename) as f:
        return [np.load(line.split('\n')[0]) for line in f]
        ...
        while True:
            for idx in indexes:
                ...
                if self.returnLandmarks:
                    ...
                    all_landmarks = [sitk_image.TransformPhysicalPointToContinuousIndex(point) for point in
                                     all_landmarks]
                    # 0-2 RV insert points
                    # 1 -> RV lateral wall turning point
                    # 3 -> LV lateral wall mid-point
                    ...
                    for i in range(0, self.agents):
                        landmarks.append(np.round(all_landmarks[i]).astype('int'))  # Apex + MV
...
class filesListBrainMRLandmark(object):
    ...
    def __init__(self, files_list=None, returnLandmarks=True, agents=2):
        ...
        if self.returnLandmarks:
            ...
            assert len(self.image_files) == len(
                self.landmark_files), 'number of image files is not equal to number of landmark files'
        sitk_image, image = NiftiImage().decode(self.image_files[0])
        self.image_dims = image.dims
    ...
    def sample_circular(self, shuffle=False):
        """ return a random sampled ImageRecord from the list of files
            ...
            replicate the image for every agent, but give him different landmarks
        """
        if shuffle:
            indexes = rng.choice(x, len(x), replace=False)
        else:
        ...
        while True:
            for idx in indexes:
                ...
                if self.returnLandmarks:
                    ...
                    # landmark = sitk_image.TransformPhysicalPoi

                    # assert self.agents % len(all_landmarks) == 0, "number of landmarks not evenly devidable by # of agents"
