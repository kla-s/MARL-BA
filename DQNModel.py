...
class Model3D(ModelDesc):
    ...
    def _build_graph(self, inputs):
        ...
        for i in range(0, self.agents):
            states.append(tf.slice(comb_state[:, i, :, :, :, :], [0, 0, 0, 0, 0], [-1, -1, -1, -1, self.channel],
                                   name='state_{}'.format(i)))  # agent 1
        ...
        for i in range(0, self.agents):
            ...
            next_states.append(tf.slice(comb_state[:, i, :, :, :, :], [0, 0, 0, 0, 1], [-1, -1, -1, -1, self.channel],
            ...
            max_pred_rewards.append(
                tf.reduce_mean(tf.reduce_max(self.predict_values[i], 1), name='predict_reward_{}'.format(i)))
        ...
        self.cost = 0.0
        for i in range(0, self.agents):
            targets.append(rewards[i] + (1.0 - tf.cast(isOver[:, i], tf.float32)) * self.gamma * tf.stop_gradient(
                best_v[i]))  # agent 1
